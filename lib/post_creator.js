...
const genAI = new GoogleGenerativeAI("AIzaSyD2y6KKo7pcPnCzuWI33-yd-Wl9O-Xc16I");
...
async function generateImage(contents) {
  ...
  async function createPost({topic = undefined, fake_news = undefined} ){
    ...
    if (fake_news === undefined) {
        fake_news = getRandomBoolean()
    }
    ...
    let prompt_2 = `Small social media post about ${result_1.response.text()}. Just one option. Include some emoji. Don't explain it, just give me the content.`;
    ...
    generateImage(`Create a realistic square photo, without embeded text, inspired by: ${result_1.response.text()}`);
