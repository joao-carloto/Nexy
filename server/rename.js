...
const thumbnailPicturesDir = path.join(
  "data",
  "thumbnails",
  "profile-pictures"
);
    ...
    db.all(query, async (err, rows) => {
      ...
      for (const row of rows) {
        ...
        if (fs.existsSync(oldProfilePicturePath)) {
          ...
          console.log(
            `Renamed: ${oldProfilePicturePath} -> ${newProfilePicturePath}`
          );
        } else {
        ...
        const updateQuery =
          "UPDATE users SET profilePictureName = ? WHERE userId = ?";
        db.run(updateQuery, [`${userId}.png`, userId], (updateErr) => {
          if (updateErr) {
            console.error(
              `Error updating database for userId ${userId}:`,
              updateErr.message
            );
          } else {
